...
const { tutorToConsultantShape } = require('./tutorPersona');

const ConversationService = require('../services/conversationService');
const ConversationMessageRepository = require('../repositories/ConversationMessageRepository');

const ChatService = {
  async getOrCreateChat(userId, tutorId) {
    return ConversationService.getOrCreateChat(userId, tutorId);
  },
};
const MessageRepository = {
  async getChatHistory(conversationId, limit) {
    return ConversationMessageRepository.getChatHistory(conversationId, limit);
  },
};
...
const GREETING_PHRASES = {
  tr: [
    ...
    'neler yapıyorsun bugün?',
    'güzel bir gün mü senin için?',
  ],
  en: [
    'how are you today?',
    'how are you doing?',
...
const GENERIC_USERNAMES = [
  'guest', 'guest user', 'user', 'anonymous', 'unknown', 'test',
  'lingola user', 'lingola', 'kullanıcı', 'kullanici',
];

function _isGenericUsername(raw) {
  if (!raw || typeof raw !== 'string') return true;
  return GENERIC_USERNAMES.includes(raw.trim().toLowerCase());
}
...
function _sanitizeGreetingName(raw) {
  ...
  if (_isGenericUsername(trimmed)) return '';
...
function resolveUserDisplayName(user, clientOverride = '') {
  const override =
    typeof clientOverride === 'string' ? clientOverride.trim() : '';
  if (!user && override) return override;
  if (!user) return '';

  const cred = user.credentialData || {};
  const credName =
    (typeof cred.name === 'string' && cred.name.trim()) ||
    (typeof cred.displayName === 'string' && cred.displayName.trim()) ||
    '';

  const candidates = [
    override,
    !_isGenericUsername(user.username) ? user.username : '',
    credName,
    user.displayName,
    user.name,
    _isGenericUsername(user.username) ? '' : user.username,
...
/** Ders greeting_opener içindeki "ismini söyle" ifadelerini profil ismi varken kaldırır. */
function _personalizeLessonOpener(opener, userName) {
  if (!opener) return opener;
  const safeUser = _sanitizeGreetingName(userName);
  if (!safeUser) return opener;

  let text = opener
    .replace(
      /\btell me your name and where you'?re from\b/gi,
      `tell me where you're from`,
    )
    .replace(/\btell me your name\b/gi, `great to meet you`)
    .replace(/\bwhat(?:'s| is) your name\b/gi, `how are you`)
    .replace(/\bask (?:for |about )?(?:your|their) name\b/gi, '')
    .replace(/\s{2,}/g, ' ')
    .trim();

  if (/introductions?/i.test(opener) && !new RegExp(`\\b${safeUser}\\b`, 'i').test(text)) {
    text = `Hi ${safeUser}! ${text.replace(/^Hi[!,.]?\s*/i, '')}`.trim();
  }
  return text;
}
...
function pickLessonGreeting(lesson, lang, { userName, coachName } = {}) {
  const opener = _personalizeLessonOpener(
    (lesson?.greetingOpener || '').trim(),
    userName,
  );
  if (!opener) return pickGreetingPhrase(lang, { userName, coachName });

  const safeUser = _sanitizeGreetingName(userName);
  const safeCoach = _sanitizeGreetingName(coachName);
  const stripped = opener.replace(/^Hi[!,.]?\s*/i, '').trim() || opener;
  const casual = pickGreetingPhrase(lang, { userName, coachName });

  const variants = [opener, stripped];
  if (safeUser) {
    variants.push(
      `Hi ${safeUser}! ${stripped}`,
      `Hey ${safeUser}, ${stripped.charAt(0).toLowerCase()}${stripped.slice(1)}`,
      `Hello ${safeUser}! ${stripped}`,
      `${casual} ${stripped}`,
    );
  } else {
    variants.push(casual, `${casual} ${stripped}`);
  }
  if (safeCoach) {
    variants.push(`Hi! I'm ${safeCoach}. ${stripped}`);
    if (safeUser) {
      variants.push(`Hey ${safeUser}, I'm ${safeCoach}. ${stripped}`);
    }
  }
  ...
  return unique[Math.floor(Math.random() * unique.length)] || opener;
}
...
class VoiceChatServerV2 {
  ...
  async _handleConnection(ws, req) {
    ...
    const connectionId = `${userId}_${tutorId}_${Date.now()}`;

    let tutor;
    try {
      tutor = await TutorService.getTutorById(tutorId);
    } catch (e) {
      console.error(`[VCv2] ❌ tutor fetch failed:`, e.message);
      ws.close(1011, 'Tutor fetch failed');
      return;
    }

    if (!tutor) { ws.close(1008, 'Tutor not found'); return; }
    const consultant = tutorToConsultantShape(tutor);
    const consultantId = tutorId;
    ...
    try {
      if (lessonId) {
        lesson = await LessonService.resolveLessonForUser(userId, lessonId, {
          skipFallback: true,
        });
      } else if (!freeTalk) {
        lesson = await LessonService.resolveLessonForUser(userId, null);
      }
      if (lesson) {
        console.log(
          `[VCv2] 📘 [${connectionId}] lesson=${lesson.id} (${lesson.cefrLevel}) ${lesson.title}`
        );
      } else if (freeTalk) {
        console.log(`[VCv2] 💬 [${connectionId}] free-talk (no fixed lesson)`);
      }
    } catch (e) {
      console.warn(`[VCv2] ⚠️ lesson load failed:`, e.message);
    }
    ...
    console.log(
      `[VCv2] ✅ ${connectionId} | user=${userId} consultant=${consultantId} ` +
      `chat=${chatId} videoMode=${!!ctx.videoMode}`
    );

    // İstemci JSON'u (call_ui_ready vb.) OpenAI kurulumu sırasında da alabilsin.
    ws.on('message', (data) => {
      if (ctx.closed) return;
      if (Buffer.isBuffer(data)) {
        if (ctx.openai?.isReady && !ctx.openaiDead) {
          const isClientPlaying =
            !!ctx.playbackFallbackTimer ||
            (ctx.turnAudioBytes > 0 && !ctx.playbackDoneReceived);
          const forwarded = !ctx.isAISpeaking && !isClientPlaying;
          if (forwarded) {
            ctx.openai.appendAudio(data);
          }
          ctx._micBytesWindow = (ctx._micBytesWindow || 0) + data.length;
          const now = Date.now();
          if (!ctx._lastMicLogAt || now - ctx._lastMicLogAt > 2000) {
            ctx._lastMicLogAt = now;
            console.log(
              `[VCv2] 🎤 [${ctx.connectionId}] mic +${ctx._micBytesWindow}B ` +
              `forwarded=${forwarded} aiSpeaking=${!!ctx.isAISpeaking} ` +
              `clientPlaying=${!!isClientPlaying} openaiReady=true`
            );
            ctx._micBytesWindow = 0;
          }
        } else {
          ctx._micBytesNoOpenai = (ctx._micBytesNoOpenai || 0) + data.length;
          const now = Date.now();
          if (!ctx._lastMicNoOpenaiLog || now - ctx._lastMicNoOpenaiLog > 2000) {
            ctx._lastMicNoOpenaiLog = now;
            console.warn(
              `[VCv2] ⚠️ [${ctx.connectionId}] mic +${ctx._micBytesNoOpenai}B ` +
              `NOT forwarded — openaiReady=${!!ctx.openai?.isReady} ` +
              `openaiDead=${!!ctx.openaiDead} closed=${!!ctx.openai?.closed}`
            );
            ctx._micBytesNoOpenai = 0;
          }
        }
      } else {
        try {
    ...
    try {
      ...
      console.log(
        `[VCv2] 🤖 [${connectionId}] OpenAI ready — model=${ctx.openai.model}`
      );

      this._sendJson(ws, { type: 'connection_success', connectionId, consultantId });

      if (ctx.videoMode) {
        ctx.awaitingUiReady = true;
        ctx.uiReadyFallbackTimer = setTimeout(() => {
          if (ctx.closed || ctx.greetingSent) return;
          console.log(
            `[VCv2] ⏱ [${ctx.connectionId}] call_ui_ready yok — karşılama (fallback)`
          );
          ctx.greetingSent = true;
      ...
      console.error(
        `[VCv2] ❌ [${connectionId}] OpenAI session setup failed:`,
        e.message
      );
      const errCode = String(e.message || '').includes('model_not_found')
        ? 'openai_model_not_found'
        : 'ai_session_setup_failed';
      this._sendJson(ws, { type: 'error', error: errCode, detail: e.message });
  ...
  _onClientJson(ctx, msg) {
    switch (msg?.type) {
      ...
      case 'playback_done':
        ...
        break;
      case 'call_ui_ready':
        if (ctx.greetingSent) break;
        if (ctx.uiReadyFallbackTimer) {
          clearTimeout(ctx.uiReadyFallbackTimer);
          ctx.uiReadyFallbackTimer = null;
        }
        ctx.awaitingUiReady = false;
        ctx.greetingSent = true;
        console.log(`[VCv2] 📱 [${ctx.connectionId}] UI ready — greeting`);
        this._triggerGreeting(ctx);
        break;
  ...
  _onPlaybackDone(ctx) {
    ...
    if (ctx.isAISpeaking) {
      console.log(
        `[VCv2] ⏭ [${ctx.connectionId}] playback_done ignored (AI still speaking)`
      );
      return;
    }

    console.log(
      `[VCv2] ✅ [${ctx.connectionId}] playback_done — scheduling idle ` +
      `(endAfter=${!!ctx.endAfterResponse} openaiReady=${!!ctx.openai?.isReady})`
    );
  ...
  _wireOpenAI(ctx) {
    ...
    openai.on('user_speech_started', () => {
      ...
      console.log(
        `[VCv2] 🗣️ [${ctx.connectionId}] OpenAI VAD speech_started ` +
        `(aiSpeaking=${!!ctx.isAISpeaking})`
      );
    ...
    openai.on('user_speech_stopped', () => {
      ...
      console.log(
        `[VCv2] 🤐 [${ctx.connectionId}] OpenAI VAD speech_stopped ` +
        `(duration=${ctx.lastSpeechDurationMs}ms)`
      );
    ...
    openai.on('user_transcript', ({ transcript }) => {
      ...
      console.log(
        `[VCv2] 📝 [${ctx.connectionId}] Whisper raw: "${raw.substring(0, 100)}"`
      );
      if (!raw) return;
  ...
  _scheduleIdleCheckin(ctx) {
    this._cancelIdleTimer(ctx);
    if (ctx.closed || ctx.openaiDead || !ctx.openai?.isReady) {
      console.warn(
        `[VCv2] ⏸ [${ctx.connectionId}] idle check-in skipped — ` +
        `closed=${!!ctx.closed} openaiDead=${!!ctx.openaiDead} ` +
        `openaiReady=${!!ctx.openai?.isReady}`
      );
      return;
    }
    ...
    console.log(
      `[VCv2] ⏳ [${ctx.connectionId}] idle timer started — ${waitMs}ms ` +
      `(checkin #${ctx.checkinCount + 1})`
    );
  ...
  async _buildSystemPrompt(
    ...
    const learnerName = resolveUserDisplayName(user, clientDisplayName);
    const safeLearner =
      _sanitizeGreetingName(learnerName) || learnerName.split(/\s+/)[0] || '';
    const nameRuleLine = safeLearner
      ? `- The learner is ${safeLearner} — use their name when natural; NEVER ask for it.\n`
    ...
    const boundaryBlock = lesson
      ? isDailyConversation
        ? `\nDAILY CHAT FOCUS:\n` +
          `- Stay on the active daily conversation topic and natural follow-ups.\n` +
          `- Briefly redirect off-topic chat back to this everyday theme.\n` +
          `- You are ${coachName}, a friendly English conversation partner.\n`
        : `\nLESSON FOCUS:\n` +
          `- Stay inside the active lesson scenario and goals.\n` +
      ...
      : freeTalk
        ...
        `instead.\n` +
        `- Do not break character. You are ${coachName}, a coach.`;
    ...
    if (safeLearner) {
      profile +=
        `\n\nLEARNER IDENTITY:\n` +
        `- Name: ${safeLearner}\n` +
        `- You ALREADY know the learner's name from their profile.\n` +
        `- Use their name naturally when it fits — do not overuse it.\n` +
        `- NEVER ask for their name, spelling, or "what should I call you".\n` +
        `- NEVER ask them to introduce themselves by name — you already have it.`;
    }
    ...
    return `${identityBlock}${lessonBlock}${freeTalkBlock}${boundaryBlock}${baseBlock}${profile}${appt}${toneBlock}`;
  }
  ...
  async _authenticate(req) {
    try {
      ...
      const tutorId = (
        url.searchParams.get('tutorId') ||
        url.searchParams.get('consultantId') ||
        ''
      ).trim().toLowerCase();
      if (!tutorId) return { success: false, error: 'Invalid tutorId' };

      const rawLang = (url.searchParams.get('lang') || '').toLowerCase().trim();
      const clientLang = /^[a-z]{2}$/.test(rawLang) ? rawLang : null;

      const videoMode =
        url.searchParams.get('video') === '1' ||
        url.searchParams.get('videoMode') === '1';

      const lessonId = (url.searchParams.get('lessonId') || '').trim() || null;
      const freeTalk = url.searchParams.get('freeTalk') === '1';

      const rawDisplayName = (
        url.searchParams.get('displayName') ||
        url.searchParams.get('learnerName') ||
        ''
      ).trim();
      const clientDisplayName =
        rawDisplayName.length > 0 && rawDisplayName.length <= 80
          ? rawDisplayName
          : '';
